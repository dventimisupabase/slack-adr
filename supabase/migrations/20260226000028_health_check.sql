-- Step 28: /adr health command — admin monitoring endpoint
-- Shows outbox queue depth, dead letters, stuck exports, and cron status.

-- 1. build_system_health: aggregate system metrics
CREATE OR REPLACE FUNCTION build_system_health() RETURNS json AS $$
DECLARE
  pending_outbox int;
  in_flight int;
  dead_letters int;
  ...
  oldest_pending_text text;
  status_emoji text;
  health_text text;
BEGIN
  -- Pending outbox (not yet delivered, not exhausted)
  SELECT count(*) INTO pending_outbox
  FROM adr_outbox
  WHERE delivered_at IS NULL
    AND attempts < max_attempts;

  -- In-flight (pg_net request sent, awaiting response)
  ...
  -- Dead letters (exhausted retries, never delivered)
  SELECT count(*) INTO dead_letters
  FROM adr_outbox
  WHERE delivered_at IS NULL
    AND attempts >= max_attempts;

  -- Stuck exports (EXPORT_REQUESTED for > 30 min)
  SELECT count(*) INTO stuck_exports
  FROM adr_events
  WHERE event_type = 'EXPORT_REQUESTED'
    AND created_at < now() - interval '30 minutes'
    AND adr_id IN (
      SELECT id FROM adrs WHERE state = 'DRAFT'
    );

  -- Age of oldest pending outbox row
  SELECT age(now(), min(created_at)) INTO oldest_pending_age
  FROM adr_outbox
  WHERE delivered_at IS NULL
    AND attempts < max_attempts;

  IF oldest_pending_age IS NOT NULL THEN
    oldest_pending_text := format('%s min', extract(epoch FROM oldest_pending_age)::int / 60);
  ELSE
    oldest_pending_text := 'n/a';
  END IF;

  -- Overall status
  IF dead_letters > 0 OR stuck_exports > 0 THEN
    status_emoji := ':warning:';
  ELSIF pending_outbox > 10 THEN
  ...
  health_text := format(
    '%s *System Health*'
    || E'\n'
    || E'\u2022 Outbox pending: %s (in-flight: %s)'
    || E'\n'
    || E'\u2022 Dead letters (failed delivery): %s'
    || E'\n'
    || E'\u2022 Stuck exports: %s'
    || E'\n'
    || E'\u2022 Oldest pending: %s',
    status_emoji,
    pending_outbox, in_flight,
    dead_letters,
    stuck_exports,
    oldest_pending_text
  );
...
-- 2. Add health to handle_slack_webhook + updated help text
