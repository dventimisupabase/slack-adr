-- Step 39: Optional reason field on state transitions
-- Usage: /adr accept ADR-2026-000001 Cost is acceptable
-- The reason is stored in adr_events.payload as {"reason": "..."}
-- Shown in /adr history and Block Kit responses

-- 1. Rewrite execute_slash_transition to extract reason from args
...
DECLARE
  ...
  adr_id_part text;
  reason_part text;
  evt_payload jsonb := NULL;
BEGIN
  IF p_adr_id_raw = '' OR p_adr_id_raw IS NULL THEN
    RETURN json_build_object(
      'response_type', 'ephemeral',
      'text', format('Usage: `/adr %s <ADR-ID> [reason]`', p_verb)
    );
  END IF;

  -- Split: first word is ADR ID, rest is optional reason
  adr_id_part := split_part(p_adr_id_raw, ' ', 1);
  reason_part := trim(substring(p_adr_id_raw FROM length(adr_id_part) + 1));

  -- Build payload with reason if provided
  IF reason_part IS NOT NULL AND reason_part != '' THEN
    evt_payload := jsonb_build_object('reason', reason_part);
  END IF;

  -- Verify team ownership
  SELECT * INTO target_adr FROM adrs a
  WHERE a.id = upper(trim(adr_id_part))
    AND a.team_id = p_team_id;

  IF NOT FOUND THEN
    RETURN json_build_object(
      'response_type', 'ephemeral',
      'text', format('ADR `%s` not found.', adr_id_part)
    );
  END IF;

  BEGIN
    PERFORM set_config('app.suppress_outbox', 'true', true);
    rec := apply_adr_event(target_adr.id, p_event_type, 'user', p_user_id, evt_payload);
...
-- 2. Rewrite build_adr_history to show reason from event payload
...
-- 3. Update help text to show [reason] syntax
